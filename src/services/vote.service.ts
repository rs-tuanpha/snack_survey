...
import { type VotingStatusResponse, type VotingStatsResponse, queryKeys } from '@/types/api'
...
export async function vote(optionId: string, topicId: string): Promise<VoteResponse> {
  const response = await api.post<VoteResponse>(`/api/vote`, { optionId, topicId })
  return response.data
}

/**
 * Unvote for a specific option within a topic
 */
export async function unvote(optionId: string, topicId: string): Promise<VoteResponse> {
  const response = await api.delete<VoteResponse>(`/api/vote`, {
    params: { optionId, topicId }
  })
  return response.data
}
...
export async function hasUserVoted(optionId: string, topicId: string): Promise<boolean> {
  ...
  const option = voteStatus.data.options.includes(optionId)
  return option
}
